...
if (selectElement && labelElement) {
  ...
  selectElement.addEventListener("change", () => {
    ...
    labelElement.textContent = `You selected: ${selectedValue.charAt(0).toUpperCase() + selectedValue.slice(1)}`
  })
}
...
class DualThumbSlider {
  ...
  drag(e) {
    ...
    const value =
      Math.round((percent * (this.config.max - this.config.min) + this.config.min) / this.config.step) *
      this.config.step

    if (this.activeThumb === "min") {
      this.config.minValue = Math.min(value, this.config.maxValue - this.config.step)
    } else {
      this.config.maxValue = Math.max(value, this.config.minValue + this.config.step)
    }
  ...
  updateSlider() {
    const minPercent = ((this.config.minValue - this.config.min) / (this.config.max - this.config.min)) * 100
    const maxPercent = ((this.config.maxValue - this.config.min) / (this.config.max - this.config.min)) * 100
    ...
    if (this.config.isPrice) {
      this.minDisplayEl.textContent = `${this.config.prefix}${this.config.minValue.toLocaleString()}`
      this.maxDisplayEl.textContent = `${this.config.prefix}${this.config.maxValue.toLocaleString()}`
    } else {
...
document.addEventListener("DOMContentLoaded", () => {
  ...
  console.log("Filter elements:", { filterBtn: !!filterBtn, filterForm: !!filterForm, overlay: !!overlay })
